...
class ProfileRepositoryTest extends PHPUnit_Framework_TestCase {
  ...
  /**
   * Test profile retrival with no matching id.
   */
  public function testProfileByNonexistingId() {
    $repo = new ProfileRepository(
      $this->getMock('\DBCDK\CommunityServices\Api\ProfileApi'),
      $this->getMock('\DBCDK\CommunityServices\Api\QuarantineApi'),
      $this->getMock('\DBCDK\CommunityServices\Api\CommunityRoleApi')
    );
    $this->assertNull(
      $repo->getProfile(1),
      'Repository does not return NULL for nonexisting profiles'
    );
  }
